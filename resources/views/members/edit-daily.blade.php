<x-layouts.app :title="__('Edit Daily Member')">
    ...
    <form action="{{ route('members.daily.update', $member->id) }}" method="POST"
        ...
        @method('PUT')
        <div class="mb-5">
            <x-forms.input type="number" label="Lock Number" name="lock_number" value="{{ $member->dailyPlan->lock_number }}"
                required />
            <x-forms.input type="number" min="0" label="Quantity" name="quantity"
                value="{{ $member->dailyPlan->quantity }}" required />
            <x-forms.input type="number" min="0" label="Price" name="price" value="{{ $member->dailyPlan->price }}"
                required />
            <x-forms.input type="number" min="0" label="Total Price" name="total_price" value="{{ $member->dailyPlan->total_price }}"
                required />
        </div>

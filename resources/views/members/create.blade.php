<x-layouts.app :title="__('Add Member')">
    ...
    <form action="{{ route('members.store') }}" method="POST"
        ...
        <div class="mb-5">
            <x-forms.input type="text" label="Full Name" name="name" required value="{{ old('name') }}" />
        </div>

        <div class="mb-5">
            <x-forms.input type="email" label="Email" name="email" value="{{ old('email') }}" />
        </div>
        ...
        <div id="daily-fields" style="display:none;">
            <div class="mb-5">
                <x-forms.input type="date" label="Date" name="date" value="{{ old('date') }}" disabled />
                <x-forms.input type="number" step="0.01" label="Price" name="price" value="{{ old('price') }}"
                    disabled />
